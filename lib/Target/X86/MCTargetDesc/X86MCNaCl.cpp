...
namespace {
unsigned getX86SubSuperRegister_(unsigned Reg, MVT::SimpleValueType VT,
                                 bool High = false);
...
static void EmitRet(const MCOperand *AmtOp, bool Is64Bit,
                    MCStreamer &Out, const MCSubtargetInfo &STI) {
...
static void EmitTrap(bool Is64Bit, MCStreamer &Out, const MCSubtargetInfo &STI) {
  // Rewrite to:
  //    X86-32:  mov $0, 0
  //    X86-64:  mov $0, (%r15)
...
static void EmitRegFix(unsigned Reg64, MCStreamer &Out, const MCSubtargetInfo &STI) {
  // lea (%rsp, %r15, 1), %rsp
...
static void EmitSPArith(unsigned Opc, const MCOperand &ImmOp,
  ...
  EmitRegFix(X86::RSP, Out, STI);
  Out.EmitBundleUnlock();
}

static void EmitSPAdj(const MCOperand &ImmOp, MCStreamer &Out, const MCSubtargetInfo &STI) {
  Out.EmitBundleLock(false);
...
static void EmitPrefix(unsigned Opc, MCStreamer &Out,
                       const MCSubtargetInfo &STI, X86MCNaClSFIState &State) {
...
static void EmitRegTruncate(unsigned Reg64, MCStreamer &Out, const MCSubtargetInfo &STI) {
  unsigned Reg32 = getX86SubSuperRegister_(Reg64, MVT::i32);
  EmitMoveRegReg(false, Reg32, Reg32, Out, STI);
}
...
static void HandleMemoryRefTruncation(MCInst *Inst, unsigned IndexOpPosition,
  ...
  if (FlagUseZeroBasedSandbox) {
    ...
    EmitRegTruncate(IndexReg, Out, STI);
  }
}
...
static void EmitTLSAddr32(const MCInst &Inst, MCStreamer &Out, const MCSubtargetInfo &STI) {
  Out.EmitBundleLock(true);
...
static void EmitREST(const MCInst &Inst, unsigned Reg32,
  ...
  Out.EmitBundleLock(false);
  if (!IsMem) {
    EmitMoveRegReg(false, Reg32, Inst.getOperand(0).getReg(), Out, STI);
  } else {
    unsigned IndexOpPosition;
    MCInst SandboxedInst = Inst;
    if (SandboxMemoryRef(&SandboxedInst, &IndexOpPosition)) {
      HandleMemoryRefTruncation(&SandboxedInst, IndexOpPosition, Out, STI);
...
bool CustomExpandInstNaClX86(const MCInst &Inst, MCStreamer &Out,
  ...
  case X86::NACL_TRAP32:
    assert(State.PrefixSaved == 0);
    EmitTrap(false, Out, STI);
    return true;
  case X86::NACL_TRAP64:
    assert(State.PrefixSaved == 0);
    EmitTrap(true, Out, STI);
    return true;
  case X86::NACL_CALL32d:
    assert(State.PrefixSaved == 0);
    EmitDirectCall(Inst.getOperand(0), false, Out, STI);
    return true;
  case X86::NACL_CALL64d:
    assert(State.PrefixSaved == 0);
    EmitDirectCall(Inst.getOperand(0), true, Out, STI);
    return true;
  case X86::NACL_CALL32r:
    assert(State.PrefixSaved == 0);
    EmitIndirectBranch(Inst.getOperand(0), false, true, Out, STI);
    return true;
  case X86::NACL_CALL64r:
    assert(State.PrefixSaved == 0);
    EmitIndirectBranch(Inst.getOperand(0), true, true, Out, STI);
    return true;
  case X86::NACL_JMP32r:
    assert(State.PrefixSaved == 0);
    EmitIndirectBranch(Inst.getOperand(0), false, false, Out, STI);
    return true;
  case X86::NACL_TLS_addr32:
    assert(State.PrefixSaved == 0);
    EmitTLSAddr32(Inst, Out, STI);
    return true;
  case X86::NACL_JMP64r:
  case X86::NACL_JMP64z:
    assert(State.PrefixSaved == 0);
    EmitIndirectBranch(Inst.getOperand(0), true, false, Out, STI);
    return true;
  case X86::NACL_RET32:
    assert(State.PrefixSaved == 0);
    EmitRet(NULL, false, Out, STI);
    return true;
  case X86::NACL_RET64:
    assert(State.PrefixSaved == 0);
    EmitRet(NULL, true, Out, STI);
    return true;
  case X86::NACL_RETI32:
    assert(State.PrefixSaved == 0);
    EmitRet(&Inst.getOperand(0), false, Out, STI);
    return true;
  case X86::NACL_ASPi8:
    assert(State.PrefixSaved == 0);
    EmitSPArith(X86::ADD32ri8, Inst.getOperand(0), Out, STI);
    return true;
  case X86::NACL_ASPi32:
    assert(State.PrefixSaved == 0);
    EmitSPArith(X86::ADD32ri, Inst.getOperand(0), Out, STI);
    return true;
  case X86::NACL_SSPi8:
    assert(State.PrefixSaved == 0);
    EmitSPArith(X86::SUB32ri8, Inst.getOperand(0), Out, STI);
    return true;
  case X86::NACL_SSPi32:
    assert(State.PrefixSaved == 0);
    EmitSPArith(X86::SUB32ri, Inst.getOperand(0), Out, STI);
    return true;
  case X86::NACL_ANDSPi32:
    assert(State.PrefixSaved == 0);
    EmitSPArith(X86::AND32ri, Inst.getOperand(0), Out, STI);
    return true;
  case X86::NACL_SPADJi32:
    assert(State.PrefixSaved == 0);
    EmitSPAdj(Inst.getOperand(0), Out, STI);
    return true;
  case X86::NACL_RESTBPm:
    assert(State.PrefixSaved == 0);
    EmitREST(Inst, X86::EBP, true, Out, STI);
    return true;
  case X86::NACL_RESTBPr:
  case X86::NACL_RESTBPrz:
    assert(State.PrefixSaved == 0);
    EmitREST(Inst, X86::EBP, false, Out, STI);
    return true;
  case X86::NACL_RESTSPm:
    assert(State.PrefixSaved == 0);
    EmitREST(Inst, X86::ESP, true, Out, STI);
    return true;
  case X86::NACL_RESTSPr:
  case X86::NACL_RESTSPrz:
    assert(State.PrefixSaved == 0);
    EmitREST(Inst, X86::ESP, false, Out, STI);
    return true;
  }
  ...
  if (SandboxMemoryRef(&SandboxedInst, &IndexOpPosition)) {
    ...
    if (PrefixLocal)
      EmitPrefix(PrefixLocal, Out, STI, State);
...
unsigned getX86SubSuperRegister_(unsigned Reg, MVT::SimpleValueType VT,
  ...
  case MVT::i8:
    if (High) {
      switch (Reg) {
      default: return getX86SubSuperRegister_(Reg, MVT::i64);
